	...
	public static void main(String[] args) {
		try {
			...
			if (cmd.hasOption("d")) {
                ...
                // for brutaltester back-compat:
                String timeoutstr = p.getProperty("timeout");
                if (timeoutstr != null)
                    System.setProperty("turn.maxtime", timeoutstr);

                String seedstr = p.getProperty("seed");
                if (seedstr != null && seedstr.length() > 0)
                    runner.setSeed( Long.valueOf( seedstr ) );

                String leaguelevel = p.getProperty("league.level");
                if (leaguelevel != null && leaguelevel.length() > 0)
                    runner.setLeagueLevel( Integer.valueOf(leaguelevel) );
			}
